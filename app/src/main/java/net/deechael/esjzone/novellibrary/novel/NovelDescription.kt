...
class NovelDescription(
    val components: List<DescriptionComponent>
)

fun analyseDescription(element: Element): NovelDescription {
    val components = mutableListOf<DescriptionComponent>()
...
private fun analyseParagraph(paragraph: Element): List<DescriptionComponent> {
    val components = mutableListOf<TextDescriptionComponent>()

    for (child in paragraph.childNodes()) {
        if (child is TextNode) {
            components.add(TextDescriptionComponent(child.text()))
        } else if (child is Element) {
            if (child.tagName() == "img") {
                return listOf(ImageDescriptionComponent(child.attr("src")))
            } else {
...
private fun analyseText(node: Node, styles: List<TextStyle>): List<TextDescriptionComponent> {
    if (node is TextNode) {
        var component = TextDescriptionComponent(node.text())
        for (style in styles)
            component = component.style(style)
        return listOf(component)
    } else if (node is Element) {
        val list = mutableListOf<TextDescriptionComponent>()

        if (node.nameIs("ruby")) {
            val rts = node.getElementsByTag("rt")
            if (rts.isNotEmpty()) {
                val furiganaComponents = mutableListOf<TextDescriptionComponent>()
            ...
            return listOf(NewLineDescriptionComponent())
...
open class TextDescriptionComponent(val text: String) : DescriptionComponent {

    private val extras: MutableList<TextDescriptionComponent> = mutableListOf()
    private val styles: MutableList<TextStyle> = mutableListOf()
    private var inline: FuriganaTextStyle? = null

    fun append(component: TextDescriptionComponent): TextDescriptionComponent {
        this.extras.add(component)
        return this
    }

    open fun style(style: TextStyle): TextDescriptionComponent {
    ...
    fun getExtras(): List<TextDescriptionComponent> {
    ...
    fun toAnnotatedString(): AnnotatedString {
        return buildAnnotatedString {
            ...
            this@TextDescriptionComponent.getStyles().forEach {
                basicSpanStyle = it.apply(basicSpanStyle)
            }


            withStyle(basicSpanStyle) {
                append(this@TextDescriptionComponent.text)
            }

            for (extra in this@TextDescriptionComponent.getExtras()) {
    ...
    fun toInlineAnnotatedString(
        ...
        return buildAnnotatedString {
            ...
            if (inline != null) {
                withStyle(basicSpanStyle) {
                    appendInlineContent(
                        this@TextDescriptionComponent.text,
                        this@TextDescriptionComponent.text
                    )
                }
                ...
                inlineContent[this@TextDescriptionComponent.text] = inline!!.inline(
                    width,
                    height,
                    basicSpanStyle,
                    this@TextDescriptionComponent.text,
...
class NewLineDescriptionComponent : TextDescriptionComponent("\n") {

    override fun style(style: TextStyle): TextDescriptionComponent {
        return this
    }

}

class ImageDescriptionComponent(val url: String) : DescriptionComponent
...
// License: MIT License
class FuriganaTextStyle(text: TextDescriptionComponent) : TextStyle {

    private val text: TextDescriptionComponent

    init {
        this.text = safer(text)
    }

    private fun safer(text: TextDescriptionComponent): TextDescriptionComponent {
        val newOne = TextDescriptionComponent(text.text)
...
private fun List<TextDescriptionComponent>.toSingle(): List<TextDescriptionComponent> {
    if (this.isEmpty())
        return listOf()
